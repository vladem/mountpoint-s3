...
fn basic_read_test(
    ...
    let size_mb = if large_file { 99 } else { 2 };
    let mut two_mib_body = vec![0; size_mb * 1024 * 1024];
    ...
    let mut two_mib_read = Vec::with_capacity(size_mb * 1024 * 1024);
...
#[test]
fn large_read_test_s3() {
    basic_read_test(fuse::s3_session::new, "large_read_test", true, false, true);
}
...
fn basic_read_test_s3(read_only: bool, pass_fuse_fd: bool) {
    basic_read_test(fuse::s3_session::new, "basic_read_test", read_only, pass_fuse_fd, false);
}
