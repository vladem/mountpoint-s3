...
use mountpoint_s3_fs::{
    ...
    manifest::{ingest_manifest, ChannelConfig, Manifest},
...
fn mount_filesystem(
    ...
    let hyperblock = Box::new(mountpoint_s3_fs::manifest::HyperBlock::new(
        make_attr_config,
        manifest.clone(),
    )?);
    ...
    let fuse_session = mp_config
        .create_fuse_session(hyperblock, client, runtime)
